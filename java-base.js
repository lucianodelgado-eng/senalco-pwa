...
function asignarEventosBase() {
    ...
    document.getElementById("btn-compartir-excel").addEventListener("click", async () => {
        const buffer = await generarExcelBuffer();
        ...
        await compartirConFallback(file, "Base Señalco Excel");
    });
...
async function compartirConFallback(file, nombre) {
  try {
    if (
      navigator.share &&
      navigator.canShare &&
  ...
  const mensaje =
    `📎 Archivo Señalco listo: ${file.name}\n\n` +
    `En WhatsApp tocá 📎 → Documento → Descargas → ${file.name}`;
  ...
  alert(
    "No se pudo abrir el menú Compartir.\n\n" +
    "Te descargué el archivo y abrí WhatsApp con el mensaje listo."
  );
}